<template>
 <div class="flex flex-col items-center md:items-baseline md:flex-row gap-2 justify-evenly [&_>_.card]:bg-base-200">
...
const percent_weekly = computed(() => {
    return ((props.weekly / props.monthly) * 100).toFixed(2)
})
const percent_daily = computed(() => {
    return ((props.daily / props.monthly) * 100).toFixed(2)
})
</script>
