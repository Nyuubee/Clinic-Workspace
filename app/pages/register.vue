<template>
    ...
    <form class="card card-normal h-max w-full max-w-xl self-center"
     ...
     >
      <div class="card-body">
        ...
        <AuthNewUsername v-model="username" :inputClass="{'input-error': duplicateUsername}">
          <template #top-right-label>
            <span class="text-error text-xl">
              <span v-if="duplicateUsername">Username is taken</span>
            </span>
          </template>
        </AuthNewUsername>
        ...
        <div class="card-actions flex flex-col items-center self-center gap-y-4">
          <button class="btn-primary btn h-full p-3 text-xl" :class="{'btn-disabled': tooWeak || duplicateUsername}">Sign Up</button>
...
const {data,error, execute:goRegister} = useAsyncData(async() => register(username.value, password.value), {
  immediate: false
})
