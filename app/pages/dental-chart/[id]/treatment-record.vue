  ...
  <NuxtLayout name="default">
    <template #navbar-start>
      ...
      <div class="join">
        ...
        <!-- Edit -->
         <!-- TODO -->
        <button class="join-item btn btn-sm btn-primary" :class="{'btn-disabled': !selected}"  onclick="treatmentForm.showModal()" @click="form?.reset(selected)">
    ...
    <dialog id="treatmentForm" class="modal">
      <div class="modal-box">
        ...
        <FormKit type="form" @submit="onSubmit" :id=FORM_ID :name="FORM_ID" :actions="false">
          <FormKit type="date" label="Treatment Date" name="createdAt" id="createdAt" :value="today" class="input input-bordered"
            placeholder="Date" />
          ...
          <FormKit type="hidden" name="dentist" id="dentist" />
          <FormKit type="text" label="Amount Charged" name="amountChargedPesos" id="amountChargedPesos" class="input input-bordered"
            placeholder="Amount Charged" />
          <FormKit type="date" label="Next appointment" name="nextAppointment" id="nextAppointment" class="input input-bordered"
    ...
    <div>
      ...
      <table class="table table-sm [&_th]:p-1 [&_td]:p-1" v-if="status == 'success'">
        ...
        <tbody>
           <RecordsTreatmentEntry v-for="treatment in treatments"  :treatment="treatment" :selected="selected?.id == treatment.id" @select="toggle(treatment)" @deselect="deselect" :class="{'bg-base-200': selected?.id == treatment.id}" />
        </tbody>
      </table>
    </div>
  </NuxtLayout>
